...
| `syllabletag`  | array of `{w, t}` where `t` is `'open'`, `'closed'`, `'name'` (vowel-consonant-e), `'team'` (vowel team) or `'bossy'` (a vowel with an r after it). The buttons shown are derived from the types the pool actually uses, so a three-type pool asks a three-way question and a five-type pool asks a five-way one; three buttons sit in one row, five wrap 2+2+1 with the last spanning the row. Each option carries its definition as a caption, since the child is learning the tags themselves | `SYLTAG15`, `SYLTAG25`, `SYLTAG31` |
| `syllable`     | array of `{w, n}` where `n` is syllable count                                | `SYLLABLES` |
| `wordchange`   | array of `{from, to}` pairs, OR `{instruction, pairs:[{from,to}]}` to customize the prompt text. Any pair may add `say` — a spoken respelling used instead of `from` for the audio (see "Making the voice say it right"). A pair may also add `hint`, a short line shown under the word — use it whenever the prompt alone would leave more than one option correct | `WORDCHANGE`, `YCHANGE`, `GUESSWORDS`, `GUESS10` |
...
| `minimalpair`  | array of `{w, other}` — `w` is spoken and is the answer, `other` is the word it contrasts with. Two options, nothing shown until the answer, so the child has to hear the difference rather than read it. Alternate which member is the target so one side is never the safe guess | `SILENTE14` |
| `sortsound`    | `{instruction, buckets:[{key, ex, why}], items:[{w, k}]}` — a "which sound do you hear?" sort, generic over its buckets (unlike `ed`, whose three are fixed). Two buckets sit side by side, three fill the row. `k` names the bucket a word belongs in; `why` is the explanation shown in feedback | `LONGU17`, `GOLDSORT29`, `EJOBS44` |
