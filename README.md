...
| `sortsound`    | `{instruction, buckets:[{key, ex, why}], items:[{w, k}]}` — a two-bucket "which sound do you hear?" sort, generic over its buckets (unlike `ed`, whose three are fixed). `k` names the bucket a word belongs in; `why` is the explanation shown in feedback | `LONGU17`, `SSOUND17` |
