...
| `syllabletag`  | array of `{w, t}` where `t` is `'open'`, `'closed'`, `'name'` (vowel-consonant-e) or `'team'` (vowel team). The buttons shown are derived from the types the pool actually uses, so a three-type pool asks a three-way question and a four-type pool asks a four-way one. Each option carries its definition as a caption, since the child is learning the tags themselves | `SYLTAG15`, `SYLTAG25` |
