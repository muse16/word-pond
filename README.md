...
| `syllabletag`  | array of `{w, t}` where `t` is `'open'`, `'closed'` or `'name'` — the three-way version of `syllabletype`, adding the Name Game (vowel-consonant-e) tag. Each option carries its definition as a caption, since the child is learning the tags themselves | `SYLTAG15` |
